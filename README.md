<p align="center">
  <img src="./frontend/assets/images/banner.png" alt="DevToCode Banner" width="100%" />
</p>

<p align="center">
  <b>Transformando conhecimento em soluções através da tecnologia e código limpo.</b>
</p>

<p align="center">
  <a href="https://devtocode.co" target="_blank" rel="noopener noreferrer">🌐 Portfólio</a> •
  <a href="https://www.linkedin.com/in/gersonluis/" target="_blank" rel="noopener noreferrer">💼 LinkedIn</a> •
  <a href="mailto:devtocode.dev@gmail.com" target="_blank" rel="noopener noreferrer">📧 E-mail</a>
</p>

---

# 👨‍💻 Sobre Mim

Olá! Sou **Luís Soares**.

Atuo no desenvolvimento de aplicações **Front-end**, **Back-end** e **Full Stack**, construindo projetos para aplicar novos conhecimentos e evoluir continuamente. 

Atualmente, concentro meus estudos e prática no ecossistema **Java & Spring Boot**, fortalecendo minha base em arquitetura de software, orientação a objetos e boas práticas de programação.

---

# 💼 Como Gosto de Desenvolver

Durante a construção dos meus projetos, busco aplicar princípios fundamentais para garantir a evolução e a qualidade do código:

- 🧹 **Código Limpo:** Preocupação em escrever um código organizado, legível e fácil de manter.
- 🏗️ **Estruturação de Projetos:** Organização clara de pastas e separação de responsabilidades.
- 🌿 **Versionamento com Git:** Histórico de alterações organizado e boas práticas com GitHub.
- 📖 **Documentação:** Cuidado em documentar o objetivo de cada projeto e como executá-lo.
- 🚀 **Aprendizado Contínuo:** Evolução diária baseada em prática e resolução de desafios reais.

---

# 💻 Tecnologias & Ferramentas

### 🌐 Front-end
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)

### ⚙️ Back-end
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)

### 🗄️ Banco de Dados
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

### 🛠️ Ferramentas
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Insomnia](https://img.shields.io/badge/Insomnia-4000BF?style=for-the-badge&logo=insomnia&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)

---

# 📊 Estatísticas do GitHub

<p align="center">
  <img height="160em" src="https://github-readme-streak-stats.herokuapp.com/?user=gersonluisoares&theme=dark" alt="GitHub Streak" />
  <img height="160em" src="https://github-readme-stats-git-masterrst-anuraghazra.vercel.app/api/top-langs/?username=gersonluisoares&layout=compact&theme=dark" alt="Linguagens mais usadas" />
</p>

---

# 📂 Projetos em Destaque

Aqui estão alguns dos principais projetos que desenvolvi para consolidar minha jornada técnica:

| Projeto | Descrição | Tecnologias | Links |
| :--- | :--- | :--- | :--- |
| **Nome do Projeto 1** | Breve resumo de 1 ou 2 linhas sobre o que a aplicação faz e o problema que resolve. | `Java` `Spring Boot` `PostgreSQL` | <a href="#" target="_blank" rel="noopener noreferrer">Repositório</a> • <a href="#" target="_blank" rel="noopener noreferrer">Deploy</a> |
| **Nome do Projeto 2** | Breve resumo de 1 ou 2 linhas sobre o que a aplicação faz e o problema que resolve. | `React` `TypeScript` `Vite` | <a href="#" target="_blank" rel="noopener noreferrer">Repositório</a> • <a href="#" target="_blank" rel="noopener noreferrer">Deploy</a> |

---

# 📈 Trilha de Especialização (Roadmap)

Organização dos meus estudos focados em consolidar uma base sólida e evoluir progressivamente:

- ✅ Lógica de Programação & Algoritmos
- ✅ Programação Orientada a Objetos
- 🚧 Collections Framework & Estrutura de Dados
- 🚧 Java Moderno
- 🚧 Spring Boot Ecosystem
- ⏳ JPA / Hibernate
- ⏳ Spring Security
- ⏳ Docker & Conteinerização
- ⏳ Testes Automatizados
- ⏳ Microsserviços

---

# 🎓 Formação & Certificados

### 🤖 Inteligência Artificial & Inovação
- **AI-Assisted Certified Professional** — Compass UOL
- **Gen AI Technical** — Compass UOL

### 💻 Desenvolvimento & Engenharia de Software
- **Formação Base para Desenvolvedor** — TreinaWeb
- **Programação Orientada a Objetos** — TIC-Hub / iRede
- **Programação Orientada a Objetos** — TreinaWeb
- **Node.js - Fundamentos** — TreinaWeb
- **Git - Fundamentos** — TreinaWeb

### 🎨 UI/UX & Processos
- **UI Design para Iniciantes** — Origamid
- **Metodologias Ágeis (XP, Scrum, Lean e Kanban)** — Udemy

---

# 📫 Vamos nos conectar?

<p align="left">
  <a href="https://www.linkedin.com/in/gersonluis/" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn Badge"/>
  </a>
  <a href="mailto:devtocode.dev@gmail.com" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/E--mail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="E-mail Badge"/>
  </a>
  <a href="https://devtocode.co" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/Portfólio-100000?style=for-the-badge&logo=Globe&logoColor=white" alt="Portfólio Badge"/>
  </a>
</p>

---

<p align="center">
  <b>"A evolução acontece quando conhecimento, prática e consistência caminham juntos."</b>
</p>
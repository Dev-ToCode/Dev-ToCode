...
const API_BASE_URL = String(window.DEVTOCODE_API_URL || "").replace(/\/+$/, "");
...
async function api(path, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    request.open(options.method || "GET", `${API_BASE_URL}${path}`, true);
    ...
    request.onload = () => {
      ...
      reject(new Error(payload.error || "Erro na requisição."));
    };

    request.onerror = () => reject(new Error("Não foi possível conectar ao servidor."));
...
if (contactForm && statusMessage) {
  contactForm.addEventListener("submit", async (event) => {
    ...
    statusMessage.textContent =
      "Mensagem registrada. Conecte este formulário ao seu serviço de e-mail quando publicar o portfólio.";
    contactForm.reset();
    await renderMetrics();
  });
}
